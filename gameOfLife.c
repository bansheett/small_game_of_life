...
/* Translate the specified x,y grid point into the index in the 
   linear array. This function implements wrapping, so
   both negative and positive coordinates that are out of the grid
   will wrap around. */
...
/* The function returns the state of the grind at x,y */
...
/* Return the number of living cells near to x,y. */
...
int main(void) {
    //inizializziamo l'array di 625 spazi, . = cellule morte, * = cellule vive.
